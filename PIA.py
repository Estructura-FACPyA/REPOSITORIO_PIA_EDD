...
def Crear_Base_de_Datos():
    try:
        with sqlite3.connect("wal-mart.db") as conn:
            mi_cursor = conn.cursor()
            # Crear la tabla Producto
            mi_cursor.execute("CREATE TABLE IF NOT EXISTS Producto (id INTEGER PRIMARY KEY, nombre TEXT NOT NULL, categoria TEXT NOT NULL, precio INTEGER NOT NULL, cantidad INTEGER NOT NULL);")
            # Crear la tabla Venta
            mi_cursor.execute("CREATE TABLE IF NOT EXISTS Venta (id INTEGER PRIMARY KEY, producto_id INTEGER NOT NULL, cantidad INTEGER NOT NULL, fecha_venta INTEGER NOT NULL, sucursal TEXT NOT NULL);")
...
def Consultar_productos():
    try:
        with sqlite3.connect("wal-mart.db") as conn:
            ...
            # Utilizamos PrettyTable para formatear la tabla
            tabla_productos = PrettyTable()
            tabla_productos.field_names = encabezados
